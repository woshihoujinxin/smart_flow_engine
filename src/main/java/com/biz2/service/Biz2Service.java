package com.biz2.service;

import com.biz2.config.FlowMappings;
import com.biz2.config.HandlerMappings;
import com.common.service.impl.AbstractThirdPartyService;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

import java.util.HashMap;
import java.util.Map;

/**
 * Created by houjinxin on 16/3/13.
 */
public class Biz2Service extends AbstractThirdPartyService {

    private static Logger logger = LoggerFactory.getLogger(Biz2Service.class);

    @Override
    protected Map<String, Object> createContext() {
        Map<String, Object> context = new HashMap<String, Object>(){{
            put("cityFlowMapping", FlowMappings._CITY_FLOW_MAPPING);
            put("stepCityPhMapping", HandlerMappings._STEP_CITY_PH_MAPPING);
            put("stepCityRhMapping", HandlerMappings._STEP_CITY_RH_MAPPING);
        }};
        return context;
    }

    @Override
    protected Logger getLogger() {
        return logger;
    }


}
